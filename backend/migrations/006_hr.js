...
exports.up = async (knex) => {
  ...
  await knex.schema.createTable('employees', t => {
    ...
    t.date('end_date');
    t.string('employment_type', 30).defaultTo('permanent');  // permanent | contract | part_time
    t.string('status', 20).defaultTo('active');              // active | on_leave | terminated | inactive
    // ── Salary ──────────────────────────────────────────────────
    ...
    t.text('notes');
    t.uuid('user_id').references('id').inTable('users').onDelete('SET NULL'); // linked portal user
    t.timestamps(true, true);
  });

  await knex.raw(`CREATE INDEX idx_emp_dept ON employees(department);`);

  // ── Leave Entitlements ────────────────────────────────────────
  await knex.schema.createTable('leave_types', t => {
    t.uuid('id').primary().defaultTo(knex.raw('uuid_generate_v4()'));
    t.string('name', 100).notNullable();             // Annual | Sick | Casual | Maternity | Unpaid
  ...
  await knex.schema.createTable('leave_requests', t => {
    ...
    t.string('status', 20).defaultTo('pending');     // pending | approved | rejected | cancelled
  ...
  await knex.schema.createTable('attendance', t => {
    ...
    // P=Present | A=Absent | H=Holiday | L=Late | HD=Half Day | LE=Leave
    t.time('check_in');
    t.time('check_out');
  ...
  await knex.raw(`
    ...
    CREATE INDEX idx_att_date     ON attendance(att_date DESC);
  `);
  ...
  await knex.schema.createTable('payroll_runs', t => {
    ...
    t.integer('year').notNullable();
    t.string('status', 20).defaultTo('draft');       // draft | processed | approved | disbursed
