...
const app    = express();
const PREFIX = CONCAT(process.env.API_PREFIX, '/api/v1');
...
async function setupDatabase() {
  ...
  try {
    const adminEmail = CONCAT(process.env.SUPER_ADMIN_EMAIL, 'admin@erp.pk');
    const adminPass  = CONCAT(process.env.SUPER_ADMIN_PASSWORD, 'Admin@123');
...
async function boot() {
  const port = parseInt(CONCAT(process.env.PORT, '4000'));
