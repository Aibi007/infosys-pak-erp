...
router.get('/', hasPermission('sales:read'), validate.query(
  ...
  if (from)       conds.push(`i.invoice_date>=${p(from)}`);
  if (to)         conds.push(`i.invoice_date<=${p(to)}`);
  if (search)     conds.push(`(i.invoice_number LIKE ${p('%'+search+'%')} OR c.name LIKE $${params.length})`);
    ...
    const result = await req.tenantDb.transaction(async (txDb) => {
      ...
      const period = await txDb.queryOne(
        `SELECT id FROM fiscal_periods WHERE status='open'
         AND $1 BETWEEN start_date AND end_date LIMIT 1`,
      ...
      const invoice = await txDb.queryOne(`
        ...
        VALUES ($1,$2,$3,COALESCE($4,CURDATE()),$5,
                $6,$7,$8,$9,$10,$11,$12,
      ...
      const glVoucher = await txDb.queryOne(`
        ...
        SELECT id, NULL, COALESCE($1,CURDATE()), $2,
               'posted', CONCAT('Sales Invoice ', $3), 'invoice', $4, $5, $5, $6
...
router.post('/:id/payments', hasPermission('sales:create'), validate(paymentSchema), async (req, res, next) => {
  ...
  try {
    await req.tenantDb.transaction(async (txDb) => {
      ...
      await txDb.execute(`CALL recalc_invoice_totals($1)`, [req.params.id]);
