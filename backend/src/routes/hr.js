...
router.get('/employees', hasPermission('hr:read'), validate.query(
  ...
  if (search) conds.push(`(e.full_name ILIKE ${p('%'+search+'%')} OR e.employee_id ILIKE $${params.length} OR e.phone ILIKE $${params.length})`);
  ...
  try {
    const employee = await req.tenantDb.transaction(async (txDb) => {
      ...
      const emp = await txDb.queryOne(`
        ...
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8::date,$9,$10,$11,$12,$13,$14::date,$15,$16)
    ...
    await req.tenantDb.transaction(async (txDb) => {
      for (const r of records) {
        await txDb.execute(`
          ...
          VALUES ($1,$2::date,$3,$4::time,$5::time,$6,$7,$8)
