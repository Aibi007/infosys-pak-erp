...
router.get('/sales', validate.query(dateRangeQuery), async (req, res, next) => {
  ...
  try {
    ...
    const groupExpr = {
      day:   `TO_CHAR(i.invoice_date, 'YYYY-MM-DD')`,
      month: `TO_CHAR(DATE_TRUNC('month', i.invoice_date), 'Mon YYYY')`,
    }[groupBy] || `TO_CHAR(DATE_TRUNC('month', i.invoice_date), 'Mon YYYY')`;
  ...
  try {
    const [statusSummary, failedList, transmitted] = await Promise.all([
      req.tenantDb.queryAll(`
        ...
        FROM invoices i
        WHERE i.invoice_date BETWEEN $1::date AND $2::date AND i.status!='voided'
      ...
      req.tenantDb.queryAll(`
        SELECT DATE_TRUNC('day', sent_at) AS day,
               COUNT(*) AS count, SUM(http_status=200::int) AS success
        FROM fbr_transmissions
        WHERE sent_at BETWEEN $1::timestamp AND $2::timestamp
