...
router.get('/', hasPermission('procurement:read'), validate.query(
  ...
  if (from)     conds.push(`po.po_date>=${p(from)}::date`);
  if (to)       conds.push(`po.po_date<=${p(to)}::date`);
  ...
  try {
    const po = await req.tenantDb.transaction(async (txDb) => {
      ...
      const po = await txDb.queryOne(`
        ...
        VALUES ($1,COALESCE($2::date,CURRENT_DATE),$3::date,'draft',$4,$5,$6,$7,$8,$9)
        RETURNING id, po_number, grand_total
...
router.post('/:id/grn', hasPermission('procurement:receive'), validate(grnSchema), async (req, res, next) => {
  ...
  try {
    const grn = await req.tenantDb.transaction(async (txDb) => {
      ...
      // Update PO status
      await txDb.execute(`SELECT update_po_receipt_status($1)`, [req.params.id]);
  ...
  try {
    await req.tenantDb.transaction(async (txDb) => {
      ...
      await txDb.execute(`
        ...
        VALUES ($1,$2,$3,$4,'payment',$5,$6,$7,COALESCE($8::date,CURRENT_DATE),$9)
