  ...
  try {
    ...
    const [
      ...
      db.queryOne(`
        ...
        FROM invoices
        WHERE DATE_FORMAT(invoice_date, '%Y-%m-01') = DATE_FORMAT(CURDATE(), '%Y-%m-01')
          AND status NOT IN ('voided','draft')
      `, []),
      ...
      db.queryAll(`
        ...
        WHERE DATE_FORMAT(inv.invoice_date, '%Y-%m-01') = DATE_FORMAT(CURDATE(), '%Y-%m-01')
  ...
  try {
    ...
    const [monthlySales, dailySales, deptPayroll] = await Promise.all([
      ...
      db.queryAll(`
        SELECT
          invoice_date AS day,
          DATE_FORMAT(invoice_date, '%d %b') AS label,
          COALESCE(SUM(grand_total),0)    AS sales,
          COUNT(*)                        AS invoices
        FROM invoices
        WHERE invoice_date >= CURDATE() - 29
          AND status NOT IN ('voided','draft')
      ...
      db.queryAll(`
        ...
        WHERE pr.year = YEAR(CURDATE())
          AND pr.month = MONTH(CURDATE())
        GROUP BY d.name
