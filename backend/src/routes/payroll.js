...
router.post('/calculate', hasPermission('hr:payroll'), validate(z.object({
  ...
  try {
    ...
    const run = existingRun
      ? await db.queryOne(`UPDATE payroll_runs SET status='draft', processed_by=$1, processed_at=NOW() WHERE id=$2 RETURNING id`, [req.userId, existingRun.id])
      : await db.queryOne(`
          INSERT INTO payroll_runs (month, year, branch_id, status, processed_by, processed_at)
          VALUES ($1,$2,$3,'draft',$4,NOW()) RETURNING id
    ...
    for (const emp of employees) {
      ...
      const salaryComps = await db.queryAll(`
        ...
        WHERE es.employee_id=$1 AND (es.effective_to IS NULL OR es.effective_to>=$2::date)
    ...
    await req.tenantDb.transaction(async (txDb) => {
      ...
      if (period) {
        ...
        const v = await txDb.queryOne(`
          ...
          SELECT id, $1, CURRENT_DATE, $2, 'posted',
            'Payroll — '||$3||'/'||$4, 'payroll', $5, $6, $6, $7
