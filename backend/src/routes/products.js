...
router.get('/', hasPermission('inventory:read'), validate.query(listQuery), async (req, res, next) => {
  ...
  try {
    ...
    if (brand) {
      params.push(`%${brand}%`);
      conditions.push(`p.brand LIKE $${params.length}`);
    }
    if (search) {
      params.push(search);
      conditions.push(`(p.name LIKE $${params.length + 0} || '%'
        OR p.sku LIKE '%' || $${params.length} || '%'
    ...
    const product = await req.tenantDb.transaction(async (txDb) => {
      ...
      if (openingStock > 0 && !hasVariants) {
        ...
        await txDb.execute(`
          ...
          VALUES ($1,$2,$3,$4)
          ON DUPLICATE KEY UPDATE qty_on_hand = stock_levels.qty_on_hand + $3
